...
| Background work | `Group.Submit`, `Drain`, and `Shutdown`; deprecated `Close(ctx)` delegates to `Drain(ctx)`. Conflicting submit scope is rejected synchronously, while the group owns concurrency, cancellation, task lifetime, and asynchronous operation-error delivery. |
